...
<%
System.out.println("TIger@@@@@@@@@@@ In View Entry JSP Folder Action");
...
<c:if test="<%= dlPortletInstanceSettingsHelper.isShowActions() %>">
	...
	>
			...
			<c:when test="<%= folder != null %>">
				...
				<c:if test="<%= hasUpdatePermission && !folder.isMountPoint() %>">
					...
					<portlet:renderURL var="copyURL">
						...
						<portlet:param name="repositoryId" value="<%= String.valueOf(repositoryId) %>" />
						<portlet:param name="rowIdsFolder" value="<%= String.valueOf(folderId) %>" />
						<portlet:param name="fromCopyURL" value="true" />
					</portlet:renderURL>
		...
		<c:if test="<%= (folder != null) && DLFolderPermission.contains(permissionChecker, scopeGroupId, folderId, ActionKeys.DELETE) %>">
			...
			System.out.println("TIger@@@@@@@@@@@ In View Entry JSP Folder Action mvcRenderCommandName 123::" +mvcRenderCommandName);
			%>
